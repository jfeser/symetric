...
module Recon = struct
  type t = Choose of t list | App of string * t list | Nonterm of string * Sexp.t | Input [@@deriving sexp]
  ...
  let rec to_program = function
    ...
    | Nonterm (n, v) -> Nonterm (`Background (n, Cad.Lang.Value.of_sexp n (Code.Sexp.sexp v)))
    | Input -> Nonterm `Input
end

let examples_of_bench = function
  | Sexp.List (_ :: List [ List inputs ] :: rest) -> (
      match List.last_exn rest with List outputs -> List.zip_exn inputs outputs | _ -> assert false)
  | _ -> assert false
...
let bench_with_examples bench exs =
  ...
  match bench with
  | Sexp.List (a :: List [ List _ ] :: b) -> Sexp.List (a :: List [ List inputs ] :: (droplast b @ [ List outputs ]))
  | _ -> assert false
  ...
  List.filter examples ~f:(fun (inp, out) ->
      let ctx =
        List.map ctx ~f:(fun (value, _, name) ->
            ...
            | `Input -> (name, Cad.Lang.Value.of_sexp "V" (Code.Sexp.sexp (Sexp.List [ inp ]))))
        |> Map.of_alist_exn (module String)
      in
      let out = Cad.Lang.Value.of_sexp "E" (Code.Sexp.sexp (Sexp.List [ out ])) in
      let out' = Cad.Lang.eval ctx prog in
      match Cad.Lang.Value.(out = out') with `Static b -> not b | `Dyn b -> not (Code.to_bool b))
...
let main ~search_exe ~bench_fn =
  ...
  let rec loop examples =
    ...
    let Unix.Process_info.{ pid; stdin; stdout; _ } = Unix.create_process ~prog:search_exe ~args:[] in
    let stdin = Unix.out_channel_of_descr stdin and stdout = Unix.in_channel_of_descr stdout in
    ...
    let failing = failing_examples full_examples @@ Recon.to_program @@ [%of_sexp: Recon.t] @@ prog in
    printf "%d failing\n" (List.length failing);
    if List.length failing = 0 then print_s prog else loop (List.hd_exn failing :: examples)
  in
  loop initial
...
let () =
  ...
  Command.basic ~summary:"Generate a CAD synthesizer."
    [%map_open
      let () = Log.param and search_exe = anon ("EXE" %: string) and bench_fn = anon ("BENCH" %: string) in
