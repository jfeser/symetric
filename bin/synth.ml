...
let main ~depth ~dump_graph ~sketch =
  ...
  Option.iter dump_graph ~f:(fun f ->
      let g = DeepSynth.search_graph depth in
      Out_channel.with_file f ~f:(fun ch -> Synth.G.output_graph ch g));

  DeepSynth.enumerate depth |> Code.to_string |> print_endline
...
let () =
  ...
  Command.basic ~summary:"Generate a synthesizer."
    [%map_open
      let () = Log.param
      and dump_graph =
        flag "-dump-graph" (optional string)
          ~doc:" produce a search graph in DOT format"
      and depth =
        flag "-depth"
