...
let main ~depth ~dump_graph ~sketch =
  let module Sketch = (val In_channel.with_file sketch ~f:Util.input_sketch : Sigs.SKETCH)
  in
...
let () =
  ...
  Command.basic ~summary:"Generate a synthesizer."
    [%map_open
      let () = Log.param
      and dump_graph =
        flag "-dump-graph" (optional string) ~doc:" produce a search graph in DOT format"
      and depth =
        flag "-depth"
