...
module Derived (C : Cstage_core.S) (B : Base with type expr = C.expr and type typ = C.typ) :
  ...
  let init len f =
    ...
    ( let_ (create t len) @@ fun a ->
      sseq [ for_ (Int.int 0) (Int.int 1) len (fun i -> set a i (genlet (f i))); a ] )
...
module ArenaArray (C : Cstage_core.S) = struct
  ...
  module Base = struct
    ...
    let mk_type e =
      let arena_type =
        Type.create ~name:(sprintf "std::array<%s, %d>" (Type.name e) default_size)
      in
    ...
    let get a x =
      eformat "($(a).ptr[$(x)])" (elem_type a.etype) "" [ ("a", C a); ("x", C x) ]
  end
