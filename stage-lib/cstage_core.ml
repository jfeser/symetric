...
module Make () : S = struct
    ...
    let main =
      {
        ...
        locals = [];
        args = [];
        fbody = { ebody = ""; ret = "0"; etype = unit_t; efree = []; eeffect = false };
      }
    in
  ...
  let to_string e =
    ...
    and var_decl_to_str v =
      ...
      match v.init with
      | Some init -> format "$(type) $(name) ($(init));" (("init", C init) :: subst)
...
|}
  ...
  let fresh_decl ?init type_ =
    ...
    match init with
    | Some v -> eformat name type_ "$(type) $(var) ($(init));" (("init", C v) :: ctx)
...
|}
      [ ("lo", C lo); ("step", C step); ("hi", C hi); ("body", S body.ebody); ("i", C i) ]
  ...
  module Sexp = struct
    ...
    module List = struct
      ...
      let init len f =
        ...
        sseq [ for_ (int 0) (int 1) len (fun i -> set a i (genlet (f i))); to_list a ]
    end
