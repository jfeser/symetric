...
module type Deps = sig
  ...
  module Tuple_3 :
    Cstage_tuple.Tuple_3.S with type 'a code := 'a t and type 'a ctype := 'a ctype

  module Tuple_4 :
    Cstage_tuple.Tuple_4.S with type 'a code := 'a t and type 'a ctype := 'a ctype
end
...
module Make (C : Deps) = struct
  module Lang = struct
    module Value = struct
      ...
      type vectors_t =
        (C.Float.t (* x *), C.Float.t (* y *), C.Float.t (* z *)) C.Tuple_3.t C.Array.t
      ...
      let vectors_t : vectors_t C.ctype =
        C.Array.mk_type @@ C.Tuple_3.mk_type C.Float.type_ C.Float.type_ C.Float.type_
      ...
      let sexp_of = function
        ...
        | Cylinder x ->
            C.Tuple_3.sexp_of x C.Int.sexp_of
              (fun s ->
                C.Tuple_4.sexp_of s C.Float.sexp_of C.Float.sexp_of C.Float.sexp_of
                  C.Float.sexp_of)
      ...
      let code_of v =
        ...
        Variants.map ~examples:conv ~vectors:conv ~sphere:conv ~cylinder:conv
          ~cylinder_offset:conv ~cuboid:conv ~cuboid_x_offset:conv ~cuboid_y_offset:conv
          ~cuboid_z_offset:conv ~bool:conv ~int:conv v
    ...
    let rec eval ctx = function
      ...
      | App ("sphere", [ s; v ]) ->
          ...
          C.Array.map vectors ~f:(fun v ->
              C3.tuple_of v @@ fun (x', y', z') ->
              C.Float.(
                ((x - x') ** float 2.0) + ((y - y') ** float 2.0) + ((z - z') ** float 2.0)
                < r ** float 2.0))
      | App ("cyl", [ c; lo; hi; v ]) ->
