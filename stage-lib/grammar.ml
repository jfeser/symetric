...
module Untyped_term = struct
  ...
  let rec to_string = function
    ...
    | App (f, []) -> f
    | App (f, xs) ->
        List.map xs ~f:to_string |> String.concat ~sep:", " |> sprintf "%s(%s)" f
...
module Rule = struct
  type 's t = { lhs : nonterm; rhs : Untyped_term.t; sem : 's list [@sexp.omit_nil] }
  [@@deriving compare, hash, sexp]
...
let rec product = function
  ...
  | s :: ss ->
      product ss |> List.concat_map ~f:(fun xs -> List.map s ~f:(fun x -> x :: xs))
...
let inline sym g =
  ...
  let rec subst_all = function
    ...
    | App (_, []) as t -> [ t ]
    | App (f, ts) ->
        List.map ts ~f:subst_all |> product |> List.map ~f:(fun ts -> App (f, ts))
...
let weighted_random ?(state = Random.State.default) l =
  ...
  let x =
    Random.State.float state 1.0 *. List.sum (module Float) l ~f:(fun (w, _) -> w)
  in
