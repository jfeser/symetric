...
type cylinder = { id : int; theta : Vector3.t; y : float; z : float; radius : float }
...
type cuboid = { id : int; theta : Vector3.t } [@@deriving compare, hash, quickcheck, sexp]
...
type sphere = { center : Vector3.t; radius : float } [@@deriving compare, hash, quickcheck, sexp]

module T = struct
  type t = Union | Inter | Sub | Sphere of sphere | Cylinder of cylinder | Cuboid of cuboid | Offset of Offset.t
...
let pp fmt = function
  ...
  | Sub -> Fmt.pf fmt "sub"
  | Sphere s -> Fmt.pf fmt "sphere_x%f_y%f_z%f_r%f" s.center.x s.center.y s.center.z s.radius
...
let args_type = function
  ...
  | Sphere _ | Offset _ -> []
  | Cylinder c -> [ Offset { id = c.id; kind = Cylinder }; Offset { id = c.id; kind = Cylinder } ]
  | Cuboid c ->
      [
