...
type box = { xmin : bound * is_open; xmax : bound * is_open; ymin : bound * is_open; ymax : bound * is_open }
...
let is_empty b =
  let is_empty_int (v, c) (v', c') = Float.(v > v' || (v = v' && is_open c && is_open c')) in
...
let quickcheck_generator =
  [%quickcheck.generator: box] |> Generator.map ~f:(fun a -> create ~xmin:a.xmin ~xmax:a.xmax ~ymin:a.ymin ~ymax:a.ymax)

let translate v = function
  | Bot -> Bot
  | Box b ->
      let add (x, c) x' = (x +. x', c) in
      create ~xmin:(add b.xmin v.Vector2.x) ~xmax:(add b.xmax v.Vector2.x) ~ymin:(add b.ymin v.Vector2.y)
        ~ymax:(add b.ymax v.Vector2.y)
...
let create_closed ~xmin ~xmax ~ymin ~ymax =
  create ~xmin:(xmin, Closed) ~xmax:(xmax, Closed) ~ymin:(ymin, Closed) ~ymax:(ymax, Closed)
...
let lub a b =
  ...
  | Bot, x | x, Bot -> x
  | Box a, Box b ->
      create ~xmin:(lo_min a.xmin b.xmin) ~xmax:(hi_max a.xmax b.xmax) ~ymin:(lo_min a.ymin b.ymin)
        ~ymax:(hi_max a.ymax b.ymax)

let glb a b =
  match (a, b) with
  | Bot, _ | _, Bot -> bot
  | Box a, Box b ->
      create ~xmin:(lo_max a.xmin b.xmin) ~xmax:(hi_min a.xmax b.xmax) ~ymin:(lo_max a.ymin b.ymin)
        ~ymax:(hi_min a.ymax b.ymax)

let%expect_test "" =
  glb
    (Box { xmin = (16.0, Closed); xmax = (18.0, Closed); ymin = (0.0, Closed); ymax = (4.0, Closed) })
    (Box { xmin = (8.0, Closed); xmax = (10.0, Closed); ymin = (0.0, Closed); ymax = (4.0, Closed) })
  |> [%sexp_of: t] |> print_s;
  [%expect {| Bot |}]
...
let graphviz_pp fmt =
  ...
  function
  | Bot -> Fmt.pf fmt "⊥"
  | Box { xmin = xminv, xminc; xmax = xmaxv, xmaxc; ymin = yminv, yminc; ymax = ymaxv, ymaxc } ->
      Fmt.pf fmt "(%.0f%ax%a%.0f &amp; %.0f%ay%a%.0f)" xminv pp_c xminc pp_c xmaxc xmaxv yminv pp_c yminc pp_c ymaxc
        ymaxv
...
let split = function
  ...
  | Box ({ xmin = xminv, _; xmax = xmaxv, _; ymin = yminv, _; ymax = ymaxv, _ } as b) ->
      ...
      let xsplit = xminv + (l / 2.0) |> round_up and ysplit = yminv + (h / 2.0) |> round_up in
      [
