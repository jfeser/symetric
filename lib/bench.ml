module Serial = struct
  type ('o, 'v) t = { ops : 'o list; output : 'v; solution : 'o Program.t option }
  [@@deriving sexp]
end
...
let param (type op value) (module Op : Sexpable.S with type t = op)
    (module Value : Sexpable.S with type t = value) =
  (module struct
    ...
    let init =
      ...
      anon ("bench" %: string)
      |> map ~f:(fun v -> Univ_map.Packed.T (key, load Op.t_of_sexp Value.t_of_sexp v))
