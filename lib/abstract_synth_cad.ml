...
let abs_value ?(range_width = 5) size =
  (module struct
    ...
    let lift : Value.t -> t Iter.t = function
      | Int _ ->
          Iter.empty
          (* Iter.(0 -- (range_width - 1)) |> Iter.map (fun offset -> Range (x - offset)) *)
      | Scene s ->
          Scene2d.to_iter size s
          |> Iter.filter_map (fun ((x, y), v) ->
                 if v then Some (Pixel_on (x, y)) else None)
    ...
    let transfer_rect ctx lx ly hx hy =
      let min = function
        | `Concrete (Value.Int x) | `Pred (Range x) -> Value.Int x
        | _ -> assert false
      in
      let max = function
    ...
    let transfer ctx (op : Cad_ext.Op.t) (args : arg list) : ret list =
      ...
      | None -> (
          ...
          | Inter, [ (`Pred (Pixel_on (x, y)) as p); `Pred (Pixel_on (x', y')) ]
            when x = x' && y = y' ->
              [ p ]
          | ( Inter,
              ( [ (`Pred (Pixel_on (x, y)) as p); `Concrete (Scene s) ]
              | [ `Concrete (Scene s); (`Pred (Pixel_on (x, y)) as p) ] ) )
            when Scene2d.(get s @@ Dim.offset size x y) ->
              [ p ]
    ...
    let eval p (v : Value.t) =
      ...
      | Pixel_on (x, y), Scene s -> Scene2d.(get s @@ Dim.offset size x y)
      | _ -> assert false
  end : Pred_intf)
