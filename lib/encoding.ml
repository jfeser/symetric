...
struct
  ...
  module App = struct
    module T = struct
      type t = Op.t * Path.t list [@@deriving compare, sexp]
    end
        ...
        |> List.fold ~init:app_cache ~f:(fun app_cache (args_v, control_var) ->
               ...
               let%bind ret_value, app_cache =
                 ...
                 | None ->
                     ...
                     in
                     return
                       (ret_value, Map.set app_cache ~key:app ~data:ret_value)
               in
               ...
               let%bind () =
                 Smt.assert_
                   Smt.(var control_var => Symb.equals ret_var ret_value)
               in
               return app_cache))
    |> Smt.ignore_m
  ...
  let check ss graph target =
    ...
    let smt =
      ...
      let%bind () =
        let expected =
          Symb.of_conc (params ss) (Bench.output (params ss).bench)
        in
