...
let pp fmt =
  match_
    ...
    ~circle:(fun x ->
      Fmt.pf fmt "circle<%.1f, %.1f, %.1f>" x.center.x x.center.y x.radius)
    ~rect:(fun x ->
      Fmt.pf fmt "rect<%.1f, %.1f, %.1f, %.1f>" x.lo_left.x x.lo_left.y x.hi_right.x
        x.hi_right.y)
...
let arity x =
  match value x with Circle _ | Rect _ -> 0 | Replicate _ -> 1 | Union | Inter -> 2
